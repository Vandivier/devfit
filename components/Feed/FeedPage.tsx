...
const NewChallenge: React.FC<NewChallengeProps> = () => {
    ...
    const exerciseOptions = exerciseList.map((exercise, index: number) => (
        {key: index, text: exercise.exercise, value: exercise.id}
    ))
    ...
    return (
        <div>
            ...
            >
                <Modal.Content>
                    <Form>
                        ...
                        <Form.Field required>
                            <label>Reps</label>
                            <input disabled={buttonLoading} type='number' onChange={(e) => setUserInput({...userInput, reps: parseInt(e.target.value)})} />
                        </Form.Field>

                        <Form.Field>
                            <label>Caption</label>
                            <input disabled={buttonLoading} type='text' onChange={(e) => setUserInput({...userInput, caption: e.target.value})} />
                        </Form.Field>
                        ...
                        <Button type='submit' fluid onClick={handleExerciseSubmit} loading={buttonLoading} disabled={buttonLoading}>Submit</Button>
                    </Form>
            ...
            <Button fluid size='big' color='pink' onClick={() => setModalOpen(!modalOpen)}>Complete a challenge</Button>
        </div>
    );
}